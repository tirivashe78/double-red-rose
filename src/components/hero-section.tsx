...
export default function HeroSection() {
  ...
  // small accessibility: ensure focus outline visible for keyboard users
  useEffect(() => {
    // nothing fancy for now — placeholder if you want to add focus management later
  }, [])

  return (
    <section id="home" className="relative h-screen overflow-hidden bg-gradient-to-b from-rose-50 via-white to-rose-100">
      {/* Background Video (keep path in public/videos/hero.mp4) */}
      <video
        src="/videos/hero.mp4"
        autoPlay
        loop
      ...
      {/* soft rose overlay to tint the video and provide consistent background */}
      ...
      {/* subtle dark layer for contrast with text */}
      <div className="absolute inset-0 bg-black/25 pointer-events-none" />

      {/* Fixed Header (does not disappear on scroll) */}
      <header className="fixed top-0 left-0 w-full z-50">
        <nav className="backdrop-blur-sm bg-black/30 text-white">
          <div className="max-w-6xl mx-auto px-4">
            <ul className="flex items-center justify-center gap-8 py-4 text-sm md:text-base font-medium">
              <li>
                <a href="#home" className="hover:text-red-400 transition-colors" aria-current="page">
                  Home
                </a>
              </li>
              <li>
      ...
      {/* Content (push down so header doesn't overlap) */}
      <div className="relative z-10 flex flex-col items-center justify-center h-full text-center px-4 pt-20">
        ...
        <motion.img
          ...
          transition={{ duration: 3, repeat: Infinity, ease: 'easeInOut', opacity: { duration: 0.8, delay: 0.2 } }}
        />
        ...
        {/* Subtitle */}
        <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1.2 }} className="mt-4 text-lg md:text-xl text-white max-w-2xl">
        ...
        {/* Scroll Indicator */}
        <motion.div animate={{ y: [0, 10, 0] }} transition={{ repeat: Infinity, duration: 1.6 }} className="absolute bottom-8 text-white text-2xl">
          <FaChevronDown />
        </motion.div>
      </div>
    </section>
